...
import { CreateAssetMaintenanceProps, AssetMaintenanceFormData } from './types';

export default function Create({ onSuccess }: CreateAssetMaintenanceProps) {
    const { t } = useTranslation();
    const { assets } = usePage<any>().props;
    const { data, setData, post, processing, errors } = useForm<AssetMaintenanceFormData>({
        asset_id: null,
        maintenance_type: '',
        title: '',
        description: '',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('assets.asset-maintenance.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent className="max-w-2xl">
            <DialogHeader>
                <DialogTitle>{t('Create Maintenance')}</DialogTitle>
            </DialogHeader>
