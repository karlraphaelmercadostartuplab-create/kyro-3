...
import { Retainer, RetainerItem } from './types';
import AuthenticatedLayout from '@/layouts/authenticated-layout';
import RetainerItemsTable from './components/RetainerItemsTable';
...
export default function Edit() {
    ...
    const { retainer, customers, warehouses } = usePage<EditProps>().props;
    const [availableProducts, setAvailableProducts] = useState([]);

    const { data, setData, put, processing, errors } = useForm({
        retainer_date: retainer.retainer_date,
        due_date: retainer.due_date,
        customer_id: retainer.customer_id.toString(),
        warehouse_id: retainer.warehouse_id?.toString() || '',
        payment_terms: retainer.payment_terms || '',
        notes: retainer.notes || '',
        items: (retainer.items || []).map(item => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('retainers.update', retainer.id));
    };
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[
                ...
                {label: t('Edit Retainer')}
            ]}
            pageTitle={t('Edit Retainer')}
        >
            <Head title={t('Edit Retainer')} />
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                <div>
                                    <Label htmlFor="retainer_date" required>
                                        {t('Retainer Date')}
                                    </Label>
                                    <DatePicker
                                        id="retainer_date"
                                        value={data.retainer_date}
                                        onChange={(value) => setData('retainer_date', value)}
                                        required
                                    />
                                    <InputError message={errors.retainer_date} />
                                </div>
                        ...
                        <CardContent>
                            ...
                            <div className="mt-6 flex justify-end">
                                <div className="w-80 bg-muted/30 rounded-lg p-4">
                                    <h3 className="font-semibold mb-3">{t('Retainer Summary')}</h3>
                                    <div>
