...
interface RetainerPaymentFilters {
    search: string;
    customer_id: string;
    status: string;
    date_range: string;
...
export default function Index() {
    ...
    const { payments, customers, bankAccounts, filters: initialFilters, auth } = usePage<IndexProps>().props;
    const urlParams = new URLSearchParams(window.location.search);

    const [filters, setFilters] = useState<RetainerPaymentFilters>({
    ...
    const [viewingItem, setViewingItem] = useState<RetainerPayment | null>(null);


    const { deleteState, openDeleteDialog, closeDeleteDialog, confirmDelete } = useDeleteHandler({
        routeName: 'retainer-payments.destroy',
    ...
    const handleFilter = () => {
        ...
        router.get(route('retainer-payments.index'), {...filterParams, per_page: perPage, sort: sortField, direction: sortDirection, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('retainer-payments.index'), {...filters, per_page: perPage, sort: field, direction, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('retainer-payments.index'), {per_page: perPage, view: viewMode});

    };

    const handleStatusUpdate = (paymentId: number, status: string) => {
        router.patch(route('retainer-payments.update-status', paymentId), { status });
    };
    ...
    const tableColumns = [
        {
            ...
            sortable: true,
            render: (value: string, payment: RetainerPayment) =>
                auth.user?.permissions?.includes('view-retainer-payments') ? (
                    <span className="text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => setViewingItem(payment)}>{value || `#${payment.id}`}</span>
                ) : (
                    value || `#${payment.id}`
                )
        },
        ...
        ...(auth.user?.permissions?.some((p: string) => ['view-retainer-payments', 'create-retainer-payments','cleared-retainer-payments', 'delete-retainer-payments','cancelled-retainer-payments'].includes(p)) ? [{
            key: 'actions',
            header: t('Actions'),
            render: (_: any, payment: RetainerPayment) => (
                <div className="flex gap-1">
                    <TooltipProvider>
                        {payment.status === 'pending' && auth.user?.permissions?.includes('cleared-retainer-payments') && (
                        ...
                        {payment.status === 'pending' && auth.user?.permissions?.includes('cancelled-retainer-payments') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
                        ...
                        {payment.status === 'pending' && auth.user?.permissions?.includes('delete-retainer-payments') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[
                {label: t('Retainer Payments')}
            ]}
            pageTitle={t('Manage Retainer Payments')}
            pageActions={
                <TooltipProvider>
                    {auth.user?.permissions?.includes('create-retainer-payments') && (
                        <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <ListGridToggle
                                ...
                                routeName="retainer-payments.index"
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            description={t('Get started by creating your first retainer payment.')}
                                            hasFilters={!!(filters.search || filters.customer_id || filters.status || filters.date_range)}
                                            onClearFilters={clearFilters}
                                            createPermission="create-retainer-payments"
                                        ...
                                        <Card key={payment.id} className="border border-gray-200 hover:shadow-md transition-shadow">
                                            <div className="p-4">
                                                <div className="flex items-center justify-between mb-3">
                                                    {auth.user?.permissions?.includes('view-retainer-payments') ? (
                                                        <h3 className="font-semibold text-base text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => setViewingItem(payment)}>{payment.payment_number || `#${payment.id}`}</h3>
                                                    ) : (
                                                        <h3 className="font-semibold text-base text-gray-900">{payment.payment_number || `#${payment.id}`}</h3>
                                                    )}
                                                    <span className={`px-2 py-1 rounded-full text-sm ${
