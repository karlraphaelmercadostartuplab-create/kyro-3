    ...
    public function updateBrandSettings(Request $request)
    {
        if(Auth::user()->can('edit-brand-settings'))
        {
            ...
            if (config('app.is_demo')) {
                ...
                $cookieParams = [];
                foreach ($themeKeys as $cookieName => $settingKey) {
                    if (isset($settings[$settingKey])) {
                        $cookieParams[] = Cookie::make($cookieName, $settings[$settingKey], 60 * 24 * 30);
                    }
                }

                $response = redirect()->back()->with('success', __('Theme settings saved in demo mode successfully.'));
    ...
    public function updateEmailSettings(Request $request)
    {
        if(Auth::user()->can('edit-email-settings'))
        {
            $validator = Validator::make($request->all(), [
                ...
                'settings.port' => 'required|integer|between:1,65535',
                ...
                'settings.port.integer' => __('SMTP port must be a valid number.'),
                'settings.port.between' => __('SMTP port must be between 1 and 65535.'),   
            ...
            $validator->after(function (ValidationValidator $validator) use ($request) {
                $host = trim((string) data_get($request->all(), 'settings.host', ''));

                if (strpos($host, '@') !== false) {
                    $validator->errors()->add('settings.host', __('SMTP host must be a mail server hostname like smtp.gmail.com, not an email address.'));
                    return;
                }
                ...
                if (!$isValidHost) {
                    $validator->errors()->add('settings.host', __('Please enter a valid SMTP host (for example: smtp.gmail.com).'));
                }
            });
    ...
    public function testEmail(Request $request)
    {
        if(Auth::user()->can('test-email'))
        {
            ...
            try {
                ...
                Log::error('Test email send failed from settings page.', [
                    'requested_to' => $request->email,
                    'message' => $exception->getMessage(),
                ]);

                return redirect()->back()->with('error', __('Failed to send test email: :error', ['error' => $exception->getMessage()]));
            }
        }
        else
        {
