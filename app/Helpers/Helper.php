        ...
        if (config('app.is_demo')) {
            ...
            foreach ($themeKeys as $cookieKey => $settingKey) {
                if (\Cookie::get($cookieKey)) {
                    $settings[$settingKey] = \Cookie::get($cookieKey);
                }
            }
        }
...
if (!function_exists('getImageUrlPrefix')) {
    ...
    {
        $storageType = admin_setting('storage_type') ?: 'local';

        switch ($storageType) {
            case 's3':
                $endpoint = admin_setting('aws_endpoint');
                if ($endpoint) {
                    return rtrim($endpoint, '/') . '/media/';
                }
                $bucket = admin_setting('aws_bucket');
                $region = admin_setting('aws_default_region');
                return "https://{$bucket}.s3.{$region}.amazonaws.com/media/";

            case 'wasabi':
                $url = admin_setting('wasabi_url');
...
if (!function_exists('SetConfigEmail')) {
    ...
    {
        if (MailConfigService::setDynamicConfig($user_id)) {
            return true;
