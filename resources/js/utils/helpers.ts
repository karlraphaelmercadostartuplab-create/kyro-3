...
const getImagePath = (path: string, pageProps?: any): string => {
  ...
  // If path is already a full storage path (e.g., /storage/media/kyro_light.png)
  if (path.includes('/storage/media/')) {
    return path.startsWith('/') ? `${window.location.origin}${path}` : `${window.location.origin}/${path}`;
  }

  // If path is a media route URL (e.g., /media/123), return it directly with origin
  if (path.match(/^\/media\/\d+($|\/|\?)/)) {
    return path.startsWith('/') ? `${window.location.origin}${path}` : `${window.location.origin}/${path}`;
  }

  // If path is a media filename route (e.g., /media/file/myimage.png), return it directly
  if (path.match(/^\/media\/file\//)) {
    return path.startsWith('/') ? `${window.location.origin}${path}` : `${window.location.origin}/${path}`;
  }

  // If it's just a filename (no slashes), serve it through storage path
  if (!path.includes('/') && !path.includes('\\') && path.includes('.')) {
    return `${window.location.origin}/storage/media/${encodeURIComponent(path)}`;
  }
  ...
  let imageUrlPrefix = pageProps?.imageUrlPrefix;
    ...
    if (isWorkdoPath) {
      ...
      // Normal case → enforce storage/media
